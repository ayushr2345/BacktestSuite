...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("DataDownloader.log", mode='w'),
...
class FivePaisaDownloader:
    ...
    def get_scrip_code_by_name(self, name: str) -> int:
        ...
        if searched_partially is True:
            logger.info (f"For Scrip name: {name}, the scrip code is partially searched with scrip code being: {result[0]}. Please verify that the correct Scrip is fetched!!! ")
        conn.close()
